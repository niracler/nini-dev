...
`design-primitives.md` SHALL be converted to `design-primitives.mdx` to support embedded Astro components.

#### Scenario: File rename

- **WHEN** migrating the file
- **THEN** MUST use `git mv` to preserve history
- **AND** the new file MUST be `src/content/blog/zh/design-primitives.mdx`
