...
- **THEN** Claude Code SHALL `cd` into the repo, run `git diff --stat` and `git status`, describe the changes to the user, and ask whether to commit, stash, or skip. If commit, use **git-workflow** skill for commit and push.
